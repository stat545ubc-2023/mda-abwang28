...
```
The class of the dataset is "tbl_df", "tbl" and "data.frame". Variable types within the table include character and double classes. There are 10,032 rows, and there are 22 variables.
...
```
The class of the dataset is "tbl_df", "tbl" and "data.frame". Variable types within the table include date, character and double classes.  There are 146,611 rows, and there are 20 variables.
...
```
The class of the dataset is "tbl_df", "tbl" and "data.frame". Variable types within the table include character and double classes. There are 3455 rows, and there are 37 variables.
...
```
The class of the dataset is "spec_tbl_df", "tbl_df", "tbl" and "data.frame". Variable types within the table include date, character and double classes. There are 20,680 rows, and there are 14 variables.
...
<!-------------------------- Start your work below ---------------------------->
**I choose the building_permits dataset. I am interested in learning more about the development patterns in Vancouver, especially given the current housing crisis. The dataset includes interesting variables, such as both the contractor and permit applicant, as well as the project value and work type.**
<!----------------------------------------------------------------------------->
...
I chose this exercise because I think it is important to understand which variables are missing the most/least values to inform future analysis. When I used glimpse on this dataset, I saw there were indeed missing values in the dataset, Knowing the number of missing values can allow me to see which variables are the most/least complete, allowing me to better tailor the questions I ask to the available data.
```{r}
building_permits %>%
...
I chose to investigate the relationship between the year and project value (represented as the mean project value). This is relevant to my overall question, as it provides a broad overview of general trends in project value over time. Additionally, the year variable has no missing values, and the project value has very few missing values, so I thought there was sufficient data to obtain a representative overview of the changes in project value over time.
```{r}
building_permits %>%
  group_by(year) %>%
...
I chose to filter observations by project value to look at only observations with high project value (greater than or equal to the 99th percentile value). This is relevant to my overall question, as it allows me to only see the most valuable permits given each year. Additionally, I selected a few variables of interest to further focus my output. 
```{r}
building_permits %>%
  group_by(year) %>%
...
I chose to look at the frequency of observations within the type of work variable grouped by year. This was relevant, because it allows me to better understand which types of work are done more/less frequently, as well as look at the distribution of the frequency of these observations by year. I chose the type_of_work variable since it is informative and does not have missing values.
```{r}
building_permits %>%
  group_by(year) %>%
...
2. What trends are there in the property use stated for the permit applications over time?
